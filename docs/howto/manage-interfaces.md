...
From the repository root, run `just interface init` and answer the prompts. Set the project name to the interface name (using underscores instead of dashes, for example `my_fancy_database`):
...
To verify that the fixture is good enough to pass the interface tests, run the `run_matrix.py` script from the `charmlibs` repo:
...
If you run this test, unless you have already merged the interface tests PR to `charmlibs`, it will fail with some error message telling you that it's failing to collect the tests for the interface, because by default, `pytest-interface-tester` will try to find tests in the `canonical/charmlibs` repo's `main` branch.

To run tests with a branch in your forked repo, run:

```bash
cd path/to/my-forked/charmlibs
python run_matrix.py --include my_fancy_database --repo https://github.com/your-github-slug/charmlibs --branch my-fancy-database
```
