...
class _Outcome(NamedTuple):
    """What one charm invocation produced."""
...
class _Runner:
    ...
    def run(self, unit_id: int, event: _Event, state: State) -> _Outcome:
...
class _InProcessRunner(_Runner):
    ...
    def run(self, unit_id: int, event: _Event, state: State) -> _Outcome:
        ...
        try:
            ...
            return _Outcome(
                cast('State', e.state), list(ctx.action_logs), _copy_results(ctx), e.message
            )
        return _Outcome(state_out, list(ctx.action_logs), _copy_results(ctx), None)
...
class _IsolatedRunner(_Runner):
    ...
    def run(self, unit_id: int, event: _Event, state: State) -> _Outcome:
        ctx = self._ctx
        try:
            state_out = ctx._run_as(unit_id, event, state)
        except ActionFailed as e:
            assert e.state is not None
            return _Outcome(
                cast('State', e.state), list(ctx.action_logs), _copy_results(ctx), e.message
...
class QueuedAction:
    """Handle for an action queued by :meth:`Juju.run`.

    ``juju run`` is synchronous, but this layer queues the action alongside
    every other event and dispatches it when the queue is drained, so the
    results do not exist yet when :meth:`Juju.run` returns. Settle first, then
    read them::

        action = juju.run(web, 'backup')
        juju.settle()
        assert action.results == {'path': '/backups/latest'}
    """

    def __init__(self, action: str, unit: Unit):
        self._action = action
        self._unit = unit
        self._logs: list[str] = []
    ...
    @property
    def done(self) -> bool:
        """Whether the action has been dispatched yet."""
        return self._done

    @property
    def logs(self) -> list[str]:
        """Messages the charm passed to ``event.log()``."""
        self._require_done()
        return self._logs

    @property
    def results(self) -> dict[str, Any] | None:
        """Results the charm passed to ``event.set_results()``."""
        self._require_done()
        return self._results

    @property
    def failure(self) -> str | None:
        """The message the charm passed to ``event.fail()``, if it failed."""
        self._require_done()
    ...
    def _require_done(self) -> None:
        if not self._done:
            raise JujuError(
                f'{self._action} has not run yet: it is queued for '
                f'{self._unit.name}. Call settle() or step() first.'
            )

    def _complete(self, outcome: _Outcome) -> None:
        self._logs = outcome.logs
    ...
    def __repr__(self) -> str:
        ...
        return f'<QueuedAction {self._action} on {self._unit.name} ({state})>'
...
class Unit:
    ...
    def state(self) -> State:
        ...
        Reading this never runs charm code: call :meth:`Juju.settle` (or
        :meth:`Juju.step`) first to drain whatever is queued, then assert.
        """
        return self._state
...
class App:
    ...
    def __init__(
        ...
        *,
        ...
        self._juju = juju
        self._name = name
        # A path runs the charm in its own interpreter; a charm class runs it
        # in this process. Which one it is decides how events are executed,
        # so the application owns that choice rather than being handed it.
        ...
        if isinstance(charm, (str, pathlib.Path)):
            self._runner = _IsolatedRunner(
                IsolatedContext(
                    ...
                    spawn_per_event=spawn_per_event,
...
class Juju:
    ...
    def deploy(
        ...
        *,
        ...
        Args:
            ...
            spawn_per_event: Run each event in a fresh worker process rather
                than a persistent one.  Isolated charms only; much slower, and
                meant for attaching a debugger.
            idle_timeout: Seconds of inactivity after which an isolated
                charm's worker process is torn down.  ``None`` keeps it until
                :meth:`close`.
            dispatch_timeout: Seconds a single event may run in an isolated
                charm's worker before it is killed.  ``None`` waits
                indefinitely, which is what a debugger needs.
    ...
    def run(
        ...
        The action is queued like any other event, so it has not run when this
        returns. Drain the queue, then read the results off the handle::

            backup = juju.run(web, 'backup')
            juju.settle()
            assert backup.results == {'path': '/backups/latest'}
        ...
        Returns:
            A :class:`QueuedAction` carrying the action's logs, results and
            failure message once it has been dispatched.
        """
        self._check_open()
        unit = target.leader if isinstance(target, App) else target
        handle = QueuedAction(action, unit)
        self._enqueue(unit.app, unit.id, _action_event(action, params), action=handle)
        return handle
    ...
    def step(self) -> Dispatch | None:
        ...
        Use this rather than :meth:`settle` to assert on state part-way
        ...
        Returns:
            ...
            empty: ``while juju.step():`` always drains it.
        """
        self._check_open()
        return self._step()
    ...
    def _step(self) -> Dispatch | None:
        ...
        but there is more queued." ``while juju.step():`` relies on that to
        drain the queue rather than stopping early.
        """
        while self._state.queue:
            app, unit_id, event, rebind, action = self._state.queue.popleft()
            ...
            outcome = app._runner.run(unit_id, event, unit._state)
            if action is not None:
                action._complete(outcome)
            state_out = outcome.state
            unit._state = state_out
            dispatch = Dispatch(event, unit, state_out)
            self._state.trace.append(dispatch)
    ...
    def _enqueue(
        ...
        self._state.queue.append(_Queued(app, unit_id, event, rebind, action))
...
def _copy_results(ctx: Context[CharmBase] | IsolatedContext) -> dict[str, Any] | None:
    """Snapshot a context's action results.

    ``Context`` clears and reuses the same dict on the next run, so the handle
    has to keep its own copy.
    """
    return dict(ctx.action_results) if ctx.action_results is not None else None
