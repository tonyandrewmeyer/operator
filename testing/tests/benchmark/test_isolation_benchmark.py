...
"""Benchmark: isolation transports on a 4-charm / 20-event workload.
...
There is no ``Model`` / ``settle`` yet, so the workload is
realised as 4 isolated charm environments, each dispatched 20 sequential events.

Acceptance bar: two decomposed **absolute** budgets, not a single ratio
...
import pytest
import yaml
from scenario._isolation import IsolatedContext
...
def _run_persistent(record: list[list[float]] | None = None) -> None:
    ctxs = [IsolatedContext(charm_source=d) for d in CHARM_DIRS]
    try:
        for ctx in ctxs:
...
def _run_spawn() -> None:
    for d in CHARM_DIRS:
        ctx = IsolatedContext(charm_source=d, spawn_per_event=True)
        try:
            events = _event_kinds(ctx.on)
            for i in range(EVENTS_PER_CHARM):
...
def test_persistent_beats_spawn_per_event(results: dict[str, float]):
    ...
    This is the regression guard — later steps re-run it to catch a
...
def test_worker_spawn_cost_under_budget(results: dict[str, float]):
    """Acceptance bar: cold worker spawn stays under budget.
...
def test_steady_state_per_event_under_budget(results: dict[str, float]):
    """Acceptance bar: warm per-event overhead stays under budget.
