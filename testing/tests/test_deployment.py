...
class MyCharm(ops.CharmBase):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        framework.observe(self.on.report_action, self._on_report)
        framework.observe(self.on.boom_action, self._on_boom)
    ...
    def _on_report(self, event: ops.ActionEvent):
        event.log('gathering')
        event.log('done')
        event.set_results({'answer': 42})

    def _on_boom(self, event: ops.ActionEvent):
        event.log('trying')
        event.fail('it went bang')
...
def test_settle_raises_when_juju_does_not_converge(juju: testing.Juju):
    ...
    with pytest.raises(testing.errors.JujuError, match='Did not converge'):
...
def test_step_dispatches_one_event_at_a_time(juju: testing.Juju):
    app = deploy_mycharm(juju)
    first = juju.step()
...
def test_state_between_steps_is_the_last_step_s_output(juju: testing.Juju):
    app = deploy_mycharm(juju)
    juju.step()  # install
    assert app.leader.state.unit_status == testing.ActiveStatus('install:info')
    juju.step()  # leader_elected
    assert app.leader.state.unit_status == testing.ActiveStatus('leader_elected:info')


def test_step_returns_none_when_the_queue_is_empty(juju: testing.Juju):
    deploy_mycharm(juju)
    juju.settle()
    assert juju.step() is None


def test_step_drains_the_queue_across_a_removed_unit(machine_juju: testing.Juju):
    # Regression test: `_step()` used to return None for a `_RemoveUnit`
    # marker as well as for an empty queue, so `while juju.step():` stopped
    ...
    while (dispatch := machine_juju.step()) is not None:
...
def test_step_drains_the_queue_across_a_vanished_rebind_target(juju: testing.Juju):
...
def test_isolated_units_share_one_worker():
    ...
    with testing.Juju(model_name='iso') as j:
        ...
        j.settle()
        # Each unit reports the PID that ran its action; one worker per
        # application means all three answer with the same one.
        actions = [j.run(unit, 'whoami') for unit in app.units]
        j.settle()
        pids: set[int] = set()
        for action in actions:
            assert action.results is not None
            pids.add(action.results['pid'])
        assert len(pids) == 1
        assert pids != {os.getpid()}  # and it is not the test process
...
def test_isolated_app_runs_config_and_run():
    ...
    with testing.Juju(model_name='iso') as j:
        ...
        action = j.run(app, 'whoami')
        j.settle()
        # Alpha doesn't observe config-changed differently, but both events
        # must round-trip the isolation boundary, and the action's results
...
def test_run_returns_a_handle_that_is_not_ready_until_settled(juju: testing.Juju):
    app = deploy_mycharm(juju)
    action = juju.run(app, 'report')
    assert not action.done
    with pytest.raises(testing.errors.JujuError, match='has not run yet'):
        _ = action.results
    juju.settle()
    assert action.done


def test_action_results_and_logs_reach_the_handle(juju: testing.Juju):
    app = deploy_mycharm(juju)
    action = juju.run(app, 'report')
    juju.settle()
    assert action.results == {'answer': 42}
    assert action.logs == ['gathering', 'done']
    assert action.failure is None


def test_a_failing_action_reports_its_message(juju: testing.Juju):
    app = deploy_mycharm(juju)
    action = juju.run(app, 'boom')
    juju.settle()
    assert action.failure == 'it went bang'
    assert action.logs == ['trying']


def test_action_handle_names_its_target(juju: testing.Juju):
    app = deploy_mycharm(juju)
    action = juju.run(app, 'report')
    assert action.name == 'report'
    assert action.unit is app.leader


def test_each_action_gets_its_own_handle(juju: testing.Juju):
    app = deploy_mycharm(juju)
    first = juju.run(app, 'report')
    second = juju.run(app, 'boom')
    juju.settle()
    assert first.results == {'answer': 42}
    assert first.failure is None
    assert second.results == {}
    assert second.failure == 'it went bang'


def test_isolated_action_results_cross_the_boundary():
    with testing.Juju(model_name='iso') as j:
        app = j.deploy(
            _ISOLATION / 'charms' / 'alpha',
            extra_sys_path=(str(_ISOLATION / 'deps' / 'confdep_v1'),),
        ...
        action = j.run(app, 'whoami')
        j.settle()
        assert action.logs == ['reporting in']
        assert action.results is not None
        assert action.results['pid'] != os.getpid()
...
# Debug knobs reach the isolated runner


def test_spawn_per_event_reaches_the_isolated_runner():
    with testing.Juju(model_name='iso') as j:
        app = j.deploy(
            _ISOLATION / 'charms' / 'alpha',
            extra_sys_path=(str(_ISOLATION / 'deps' / 'confdep_v1'),),
        ...
        j.settle()
        runner = app._runner
        assert isinstance(runner, _deployment._IsolatedRunner)
        assert runner._ctx._spawn_per_event
        assert runner._ctx._worker is None  # never spawns a persistent one


def test_idle_and_dispatch_timeouts_reach_the_isolated_runner():
    with testing.Juju(model_name='iso') as j:
        app = j.deploy(
            _ISOLATION / 'charms' / 'alpha',
            extra_sys_path=(str(_ISOLATION / 'deps' / 'confdep_v1'),),
        ...
        runner = app._runner
        assert isinstance(runner, _deployment._IsolatedRunner)
        assert runner._ctx._idle_timeout == pytest.approx(30.0)
        assert runner._ctx.dispatch_timeout == pytest.approx(12.0)
