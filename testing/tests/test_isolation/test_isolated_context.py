...
"""Tests for IsolatedContext, which runs one charm in a worker subprocess."""
...
import pytest
from scenario import ActiveStatus, State
from scenario._isolation import IsolatedContext, _IsolatedEnv, _load_charm_spec
from scenario.errors import IsolationError, MetadataNotFoundError
...
def test_alpha_charm_sees_confdep_v1():
...
def test_beta_charm_sees_confdep_v2():
...
def test_independent_runs_see_different_dep_versions():
...
def test_multiple_events_on_same_context():
...
@_VENV_TEST_MARKER
def test_alpha_in_own_venv(tmp_path):
...
@_VENV_TEST_MARKER
def test_conflicting_charms_in_separate_venvs(tmp_path):
...
def test_missing_dep_raises_isolation_error():
...
def test_isolation_error_includes_worker_traceback():
...
def test_dispatch_timeout_kills_the_worker():
...
def test_nonexistent_charm_source_raises_value_error():
...
def test_isolated_env_is_frozen():
...
def test_isolated_env_custom_python_executable():
...
def test_isolated_env_extra_sys_path():
...
def test_load_charm_spec_reads_metadata_yaml():
    """_load_charm_spec reads the legacy metadata.yaml."""
    meta, _, _ = _load_charm_spec(CHARMS / 'alpha')
    assert meta['name'] == 'alpha'


def test_load_charm_spec_splits_a_unified_charmcraft_yaml(tmp_path: pathlib.Path):
    """Config and actions in a unified charmcraft.yaml come back separately."""
    ...
    meta, config, actions = _load_charm_spec(tmp_path)
    assert meta['name'] == 'unified'
    assert 'config' not in meta
    assert 'actions' not in meta
    assert config == {'options': {'port': {'type': 'int', 'default': 80}}}
    assert actions == {'backup': {}}


def test_isolated_context_app_name_from_metadata():
    """IsolatedContext defaults app_name to the charm name from metadata."""
    ctx = IsolatedContext(
        charm_source=CHARMS / 'alpha',
        extra_sys_path=(str(DEPS / 'confdep_v1'),),
    )
    assert ctx.app_name == 'alpha'


def test_isolated_context_explicit_app_name():
    ...
    assert ctx.app_name == 'my-alpha'


def test_isolated_context_explicit_meta():
...
def test_nonexistent_metadata_raises(tmp_path: pathlib.Path):
    """_load_charm_spec raises MetadataNotFoundError if no metadata is found."""
    # Create a charm directory with no metadata files.
    (tmp_path / 'src').mkdir()
    with pytest.raises(MetadataNotFoundError, match='metadata'):
        _load_charm_spec(tmp_path)


def test_juju_version_reaches_the_worker():
    """The juju_version given to IsolatedContext is the one the charm observes."""
    ctx = IsolatedContext(charm_source=CHARMS / 'jujuver', juju_version='3.4.2')
    state_out = ctx.run(ctx.on.install(), State())
    assert state_out.unit_status == ActiveStatus('3.4.2')


def test_app_trusted_reaches_the_worker():
    """The app_trusted given to IsolatedContext is what the charm sees."""
    untrusted = IsolatedContext(charm_source=CHARMS / 'envcheck')
    trusted = IsolatedContext(charm_source=CHARMS / 'envcheck', app_trusted=True)
    untrusted_out = untrusted.run(untrusted.on.install(), State())
    trusted_out = trusted.run(trusted.on.install(), State())
    assert untrusted_out.unit_status.message.startswith('untrusted ')
    assert trusted_out.unit_status.message.startswith('trusted ')


def test_charm_root_reaches_the_worker(tmp_path: pathlib.Path):
    """The charm_root given to IsolatedContext is the charm's directory."""
    ctx = IsolatedContext(charm_source=CHARMS / 'envcheck', charm_root=tmp_path)
    state_out = ctx.run(ctx.on.install(), State())
    assert state_out.unit_status == ActiveStatus(f'untrusted {tmp_path}')
